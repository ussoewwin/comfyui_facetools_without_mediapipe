...
# Install packages.
# setuptools>=82 removed pkg_resources; use importlib.metadata instead.

import subprocess
from importlib.metadata import distributions

def _pkg_key(name: str) -> str:
    return name.lower().replace("_", "-")

cur_dir = os.path.dirname(__file__)
with open(os.path.join(cur_dir, 'requirements.txt')) as f:
    required_packages = {_pkg_key(line.strip()) for line in f if line.strip()}
    installed_packages = {
        _pkg_key(dist.metadata["Name"])
