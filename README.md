...
- **UssoewwinImageResizeAdvanced**: Advanced image resizing with multiple scaling modes and aspect ratio control. Supports three action types: resize only, crop to ratio, and pad to ratio. Provides flexible scaling options via smaller_side, larger_side, or scale_factor (only one can be active at a time). Includes resize mode selection (reduce size only, increase size only, or any) to control scaling direction. Supports aspect ratio adjustment via side_ratio string (e.g., "4:3") with configurable crop/pad position (0.0-1.0) for precise placement. Features pad feathering (0-8192) for smooth mask transitions when padding. Returns both processed image and corresponding mask. Optional mask input allows preserving existing masks during transformations.
- **FacetoolsConstrainImage (`Constrain Image (Facetools)`)**: Constrains an image to a maximum and minimum bounding box while strictly preserving aspect ratio, with optional center cropping.
  - **Why this node was built (Fixing upstream crashes)**: Upstream implementations like `ConstrainImage` from `ComfyUI-Custom-Scripts` (pysssss) suffer from a critical flaw: when `max_width`, `max_height`, `min_width`, or `min_height` receive `0` (e.g. from upstream INT/primitive nodes or when intending "unconstrained"), the upstream node calculates `constrained_height = min(..., 0) = 0` and immediately crashes the entire workflow with `ZeroDivisionError: division by zero` on `constrained_width / constrained_height`. `FacetoolsConstrainImage` safely treats `0` as "unconstrained / use image dimension" and includes strict division-by-zero guards across all aspect ratio calculations, guaranteeing crash-free execution in all workflow configurations.
  - **Parameters**:
    - `images`: Input image tensor batch.
    - `max_width` / `max_height`: Maximum width/height boundary (`0` = unconstrained).
    - `min_width` / `min_height`: Minimum width/height boundary (`0` = unconstrained).
