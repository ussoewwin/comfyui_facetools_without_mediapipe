...
- **SAM3SegmentV3**: Text-prompt based segmentation using SAM3 (Segment Anything Model 3). Adapted from [ComfyUI-RMBG](https://github.com/1038lab/ComfyUI-RMBG)'s SAM3 segmentation node. Uses natural language prompts to segment objects in images. Supports configurable confidence threshold (0.05-0.95), mask blur (0-64), mask offset (-64 to 64), invert output, background options (Alpha/Color), and device selection (Auto/CPU/GPU). Returns segmented image, mask tensor, and mask image. Model is automatically downloaded on first use. Note: This node is optional and will not prevent the extension from loading if SAM3 dependencies are missing. Original Repository: [ComfyUI-RMBG](https://github.com/1038lab/ComfyUI-RMBG) (Please check the original repository for license information)
