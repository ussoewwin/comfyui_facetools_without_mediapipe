...
def onnx_inference(lip_session, input_image, mask_components=[0]):
    # input_image can be PIL Image or directory path
    ...
    dataset_lip = SimpleFolderDataset(root=input_image, input_size=input_size, transform=transform)
    dataloader_lip = DataLoader(dataset_lip)
    palette = get_palette(20)
    with torch.no_grad():
        for _, batch in enumerate(tqdm(dataloader_lip, desc="Processing")):
            image, meta = batch
